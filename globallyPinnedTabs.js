...
GloballyPinnedTabs.create = function (serializedTabs) {
    ...
    var result = new GloballyPinnedTabs(
        serializedTabs.map(
            function (x) {
...
GloballyPinnedTabs.prototype = {
    ...
    addTab: function (tab) {
        var self = this;
        var newTab = new GloballyPinnedTab(
            tab.url, tab.url, tab.favIconUrl, tab.title, function (pinnedTab) {
    ...
    activateWindow: function (window, callback) {
        ...
        if (oldWindowId !== undefined) {
            var self = this;
            Chrome.getWindow(
                oldWindowId, function (oldWindow) {
    ...
    moveTabs: function (targetWindow, sourceWindow, callback) {
        ...
        var performMove = function () {
            Chrome.moveTabs(
                realTabIds, targetWindow.id, smallestRealTabsIndex, function (tabs) {
                    if (dummyTabs.missingTabs.length) {
                        Chrome.getWindow(
                            sourceWindow.id, function (window) {
            ...
            if (dummyTabIds.length) {
                Chrome.moveTabs(
                    dummyTabIds, sourceWindow.id, smallestDummyTabsIndex, function (tabs) {
                        for (var i = 0; i < tabs.length; ++i) {
        ...
        if (sourceWindow.tabs.length === this.tabs.length) {
            Chrome.createPinnedTab(
                sourceWindow, 'about:blank', function (tab) {
                    temporaryTabIds.push(tab.id);
